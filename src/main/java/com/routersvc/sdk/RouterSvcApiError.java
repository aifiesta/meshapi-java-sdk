...
/**
 * Thrown when RouterSVC returns a non-2xx response or sends an error frame mid-stream.
 */
public class RouterSvcApiError extends RuntimeException {
    ...
    public RouterSvcApiError(String message, int status, String errorCode,
    ...
    public String toString() {
        return String.format("RouterSvcApiError(status=%d, code=%s, requestId=%s): %s",
    ...
    /**
     * Parses a RouterSvcApiError from an HTTP response.
     * Handles non-JSON bodies gracefully.
     */
    public static RouterSvcApiError fromResponse(HttpResponse<String> response) {
        ...
        if (!contentType.contains("application/json")) {
            ...
            return new RouterSvcApiError(msg, status, "parse_error", requestId, null, null);
        }
        ...
        try {
            ...
            return new RouterSvcApiError(message, status, code, reqId, details, retryAfter);
        } catch (Exception e) {
            String msg = body != null && body.length() > 500 ? body.substring(0, 500) : body;
            if (msg == null || msg.isBlank()) msg = "HTTP " + status;
            return new RouterSvcApiError(msg, status, "parse_error", requestId, null, null);
    ...
    public static RouterSvcApiError streamInterrupted(String cause) {
        return new RouterSvcApiError("Stream interrupted: " + cause,
                0, "stream_interrupted", "", null, null);
    }

    /** Creates an error from an in-stream error frame. */
    public static RouterSvcApiError fromStreamFrame(String code, String message) {
        return new RouterSvcApiError(message, 0,
