...
class ErrorMappingTest {
    ...
    void fromResponse_401() {
        ...
        RouterSvcApiError err = RouterSvcApiError.fromResponse(makeResponse(401, body, "application/json"));
    ...
    void fromResponse_429WithRetryAfter() {
        ...
        RouterSvcApiError err = RouterSvcApiError.fromResponse(makeResponse(429, body, "application/json"));
    ...
    void fromResponse_422WithDetails() {
        ...
        RouterSvcApiError err = RouterSvcApiError.fromResponse(makeResponse(422, body, "application/json"));
    ...
    void fromResponse_htmlBody_parseError() {
        RouterSvcApiError err = RouterSvcApiError.fromResponse(makeResponse(502, "<html>Bad Gateway</html>", "text/html"));
    ...
    void fromResponse_malformedJson_parseError() {
        RouterSvcApiError err = RouterSvcApiError.fromResponse(makeResponse(500, "{not json}", "application/json"));
        assertEquals("parse_error", err.getErrorCode());
    }

    @Test
    void streamInterrupted_factory() {
        RouterSvcApiError err = RouterSvcApiError.streamInterrupted("connection reset");
    ...
    void fromStreamFrame_factory() {
        RouterSvcApiError err = RouterSvcApiError.fromStreamFrame("upstream_error", "provider error");
        assertEquals("upstream_error", err.getErrorCode());
    }

    @Test
    void isRuntimeException() {
        RouterSvcApiError err = RouterSvcApiError.fromStreamFrame("x", "y");
        assertInstanceOf(RuntimeException.class, err);
    }

    @Test
    void toString_containsStatusAndCode() {
        RouterSvcApiError err = new RouterSvcApiError("msg", 404, "not_found", "req_x", null, null);
